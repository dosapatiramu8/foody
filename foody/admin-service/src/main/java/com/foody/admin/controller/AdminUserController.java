...
public class AdminUserController {
    ...
    @GetMapping("/{id}")
    public Mono<AdminUserResponse> getAdminUser(@PathVariable String id) {
        return adminUserService.getAdminUserById(id);
    }

    @PutMapping("/{id}")
    public Mono<AdminUserResponse> updateAdminUser(@RequestBody AdminUserRequest adminUserRequest) {
        return adminUserService.updateAdminUser( adminUserRequest);
    }

    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public Mono<Void> deleteAdminUser(@PathVariable String id) {
