...
public class MongoTemplateUtil {
    ...
    public static final String DATA_BASE_CONNECTION_ERROR = "Database connection error, ";
    ...
    public <T, E> Supplier<Flux<T>> aggregation(TypedAggregation<T> aggregation, Class<T> type, Class<E> entityType) {
        ...
        //log.info("DB_QUERY ");
        return () -> reactiveMongoTemplate.aggregate(newAggregation, collectionName, type).onErrorResume(throwable -> Flux.error(new AppDataBaseException(HttpStatus.BAD_GATEWAY, DATA_BASE_CONNECTION_ERROR + throwable.getMessage()))).contextWrite(context -> Context.of(MDC.getCopyOfContextMap()));
    }

    public <T> Supplier<Flux<T>> aggregation(TypedAggregation<T> aggregation, String collectionName, Class<T> entityType) {

        Aggregation newAggregation = allowDiskUse(aggregation);
        return () -> reactiveMongoTemplate.aggregate(newAggregation, collectionName, entityType).onErrorResume(throwable -> Flux.error(new AppDataBaseException(HttpStatus.BAD_GATEWAY, DATA_BASE_CONNECTION_ERROR + throwable.getMessage())));
    }
    ...
    public <T> Supplier<Flux<T>> aggregation(TypedAggregation<T> aggregation, Class<T> type) {
        ...
        Aggregation newAggregation = allowDiskUse(aggregation);
        return () -> reactiveMongoTemplate.aggregate(newAggregation, collectionName, type).onErrorResume(throwable -> Flux.error(new AppDataBaseException(HttpStatus.BAD_GATEWAY, DATA_BASE_CONNECTION_ERROR + throwable.getMessage())));
    }
